...
namespace CloudAutomation.Resources.VirtualMachine.Processors
{
    public class ResourceProcessor<T> : IResourceProcessor<T> where T : class
    {
        ...
        public async Task<T> Execute(Resource resource, string username, string password)
        {
            ...
            var isCreated = _azureProcessor.CreateResource(azureLogin, resourceData, resourceGroup, username, password, out var createdResource);
